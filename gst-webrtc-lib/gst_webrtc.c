...
char * build_pipeline(ApplicationCtx * app_ctx_p, JsonArray *streams, const char *stunserver) {
  ...
  GError *error = NULL;
  InstanceCtx *ctx;
  char *pipeline_str, *name;
  char *tmp_str = g_strconcat("webrtcbin bundle-policy=max-bundle name=sendrecv ", STUN_SERVER_PREFIX, stunserver, " ", NULL);
