...
static void send_ice_candidate_message(GstElement * webrtc G_GNUC_UNUSED, guint mlineindex, gchar * cand, gpointer user_data G_GNUC_UNUSED) {
  ...
  json_object_set_string_member(msg, "to", g_peer_id);
 ...
 * Return:                                                                                   *
 *********************************************************************************************/
gboolean start_pipeline(InstanceCtx * ctx) {
  GstStateChangeReturn ret;
  GError *error = NULL;
  
  char* pipeline_str = g_strconcat("webrtcbin bundle-policy=max-bundle name=sendrecv ", STUN_SERVER_PREFIX, g_stunserver, " ", ctx->pipeline_str,"! sendrecv. ", NULL);
  // g_print("Pipeline: %s\n", pipeline_str);
  
  /* char* pipeline_str = g_strconcat("webrtcbin bundle-policy=max-bundle name=sendrecv ", STUN_SERVER,
